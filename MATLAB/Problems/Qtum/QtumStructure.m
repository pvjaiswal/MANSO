function [minmax, d, m, VarNature, VarBds, FnGradAvail, NumConstraintGradAvail, StartingSol, budget, ObjBd, OptimalSol, NumRngs] = QtumStructure(NumStartingSol)
...
VarBds = [zeros(d,1),ones(d,1)*pi]; % bounds from the description
FnGradAvail = 0; % IPA derivative is coded
...
if (NumStartingSol < 0) || (NumStartingSol ~= round(NumStartingSol))
    ...
    if (NumStartingSol == 0)
        ...
        StartingSol = zeros(1,d) + pi * rand(d, NumStartingSol)';
    end    
end
