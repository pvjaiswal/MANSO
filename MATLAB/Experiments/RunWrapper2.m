...
for k1 = 1:length(problemnameArray)
    ...
    for k2 = 1:length(solvernameArray)   
        ...
        %end
        % This does not works when using python
...
%         FnVarMatrix = cat(1, AFnVar_cell{:});
        ...
        %Concatenate cell data across macroreplications
        BudgetMatrix =Ancalls_cell{1};
        %SolnMatrix = A_cell{1};
        FnMeanMatrix = AFnMean_cell{1};
        FnVarMatrix = AFnVar_cell{1};
        SolnMatrix = ptdict.pts;
        FnMeanVal =  ptdict.means;
        FnCount = ptdict.counts;
...
%             fprintf('\t Overwriting \t --> ')
%         end
%         save(strcat(pwd,'/RawData/RawData1234_',solvername,'_on_',problemname,'.mat'), 'BudgetMatrix', 'SolnMatrix', 'FnMeanMatrix', 'FnVarMatrix');
