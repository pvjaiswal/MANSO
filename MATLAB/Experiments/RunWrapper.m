function RunWrapper(problemnameArray, solvernameArray, repsAlg,Ne)
...
for k1 = 1:length(problemnameArray)
    ...
    for k2 = 1:length(solvernameArray)   
        ...
        solnsfilename = strcat('RawData_',solvername,'_on_',problemname,string(Ne),'.mat');
        if exist(strcat('RawData/',solnsfilename), 'file') == 2
            fprintf('\t Overwriting \t --> ')
        end
        save(strcat(pwd,'/RawData/RawData234_',solvername,'_on_',problemname,string(Ne),'.mat'), 'BudgetMatrix', 'SolnMatrix', 'FnMeanMatrix', 'FnVarMatrix');
