...
function [Ancalls, Asoln, Afn, AFnVar, AFnGrad, AFnGradCov, ...
    ...
    budgetini=0;
    %min(30,max(floor(0.001*budgetmax)+1,5));
    tic;
    Start=0;
    %[fxini, varini, ~, ~, ~, ~, ~, ~] = probHandle(x0, budgetini, problemRng, 1);
    %x0=xinit;
    budgetmaxPT = budgetmax;
    End=toc;
    %fprintf('End of Stage 1- Number of calls made %d, Time duration %f \n',budgetini,End-Start);
    Start=End;
    ...
    setupprct=.05; %the initial percentage of the budget to use for finding a good initial radius
    ...
    for i=1:length(radiustry)
        ...
        %fprintf('End of Stage 2-%d: Number of calls made %d, Time duration %f \n\n',i,infoi.calls,End-Start);
        Start=End;
    ...
    if size(x_aft_tune,1)==2 %% PJ removed bug two best points.
        x_aft_tune = x_aft_tune(1,:);
    end
    ...
    % We donot need initial points and there fn values, so I removing it
     Asoln = [x_points_par; x_points]; % x0 removed
     Afn = [func_points_par; func_points]; % fxini removed
     AFnVar = [var_points_par; var_points]; % varini removed
     Ancalls = [callcount_par; callcount]; % budgetini removed
%        disp(budgetini)
%        disp(callcount_par)
%        disp(callcount)
end
...
%           .sensitivity = the sensitivity of the variable bounds
    tim=0;
    x_init = xk;
    %fprintf("Error in Ptdict, its size is %d %d",size(xk));
    % Separate the two solver random number streams
    solverInitialRng = solverRng{1}; % RNG for finding initial solutions
    ...
    % More default values
    eta_1 = 0.10;           %threshhold for decent success-PJ changed from 0.1
    eta_2 = 0.50;           %threshhold for good success- PJ changed from 0.5
    w = 0.99; 
    mu = 1.05; 
    beta = 1/mu;
    gamma_1 = (1.25)^(2/dim);  %successful step radius increase Changed from (1.25)^(2/dim)
    ...
    while calls <= budgetmax
        ...
        if size(xk,1)==2
            fprintf("Error in Center, its size is %d %d \n",size(xk));
        end
        ...
        %x_tilde
        End=toc;
        %fprintf('Time to minimize the model %f \n',End-Start)
        Start=End;
        %Step 4
        ...
        % Added following lines to fix a bug in computing rho (to account for 0/0 = NaN )-Prateek Jaiswal
        
        F_red= (Fbar(1) - Fbar_tilde);
        M_red= (Model_Approximation(xk-xk,lin_quad,q) - Model_Approximation(x_tilde-xk,lin_quad,q));
        ...
        if size(x_tilde,1)==2 %% PJ removed bug, picking one of the two best points.
            fprintf('picking one of the two best points\n');     
            x_tilde = x_tilde(1,:);
        end
        ...
        if rho >= eta_2 %really good accuracy
            ...
            xk = x_tilde;
            %fprintf('%f %f %f %d %f \n',x_tilde,calls,Fbar_tilde); %% Added to check progress##Prateek
            %fprintf('%d %f \n',calls,Fbar_tilde); %% Added to check progress##Prateek
    ...
    if isempty(x_points) == false
       ...
       % disp(4)
        ...
        %disp(func_points)
        %fprintf('End Stage: %f %f %f %d %f \n',x_points(end,:),callcount(end),func_points(end)); %% Added to check progress##Prateek
...
%
    ...
    [~, dim] = size(center);
    %fprintf("Error in Ptdict, its size is %d %d",size(center));
...
%
    ...
    lambdak = 20*log10(k)^1.5; %10*log10(k)^1.5;
    kappa = 10^3;
