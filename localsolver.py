...
def ASTRODF(x,leng,Problem,bounds,eng):
    ...
    Output={'Points': np.array(Result[0])[sort][0],
            'Values': (np.array(Result[1])[sort][0]),
            'Counts': (np.array(Result[2])[sort][0])}
    return (Output)
...
def Bobyqa(x,leng,Problem,bounds):
    ...
    Output={'Points': history[:,np.linspace(1,len(x),len(x)).astype(int)][sort],
            'Values': history[:,0][sort],
            'Counts': [[1]]*len(history[:,0][sort]) }
